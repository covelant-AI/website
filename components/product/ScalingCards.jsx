...
import { getCards } from "@/data/StaticData";
import { useTranslations } from "next-intl";

export default function ScalingCards() {
  const t = useTranslations("components.product.scalingCards");
  const cards = getCards(t); // ← localized titles/descriptions
    ...
    <section className="relative flex flex-col items-center text-center py-16 px-6 font-Figtree">
      ...
      <div className="flex flex-col md:flex-row gap-6 w-full max-w-5xl justify-center">
        {cards.map((card, index) => (
          ...
          >
            <Image src={card.image} alt={card.title} width={300} height={300} className="mb-4" />
